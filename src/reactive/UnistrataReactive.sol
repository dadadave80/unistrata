...
/// @title UnistrataReactive
/// @author David Dada (https://github.com/dadadave80)
/// @notice Reactive Smart Contract (deployed on Reactive Network / Lasna) that automates Unistrata's
///         epoch settlement with zero off-chain infrastructure (brief §3.7):
///         1. a CRON heartbeat → `settleEpoch(address)` once per epoch;
///         2. the hook's `UnistrataObservation` → `emergencySettle(address)` on a variance spike.
...
contract UnistrataReactive is AbstractReactive {
    error UnistrataReactive__NotOwner();
    ...
    address public immutable unistrataHook;
    ...
    constructor(
        ...
        if (!vm) {
            // CRON heartbeat (on the Reactive chain) + UnistrataObservation (on the origin chain).
    ...
    function subscribeAll() external {
        if (msg.sender != owner) revert UnistrataReactive__NotOwner();
        if (vm) return;
    ...
    function _emitCallback(string memory signature) internal {
        emit Callback(originChainId, unistrataHook, callbackGasLimit, abi.encodeWithSignature(signature, address(0)));
    }
}
