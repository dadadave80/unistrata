...
contract DeployStrataScript is BaseScript {
    ...
    function run() public {
        ...
        (Currency currency0, Currency currency1, uint8 dec0, uint8 dec1, bool numeraireIsToken1) =
            StrataDeploy.orderTokens(weth, 18, usdc, 6, usdc);
        ...
        uint160 sqrtPriceX96 = wethIsToken0
            ? StrataDeploy.encodeSqrtPriceX96(targetPrice * 1e6, 1e18)  // (amount1=USDC, amount0=WETH)
            : StrataDeploy.encodeSqrtPriceX96(1e18, targetPrice * 1e6); // (amount1=WETH, amount0=USDC)
